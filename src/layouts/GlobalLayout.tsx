          ...
          <div className="flex items-center gap-10 h-full">
            ...
            {isAppPage ? (
              <div className="hidden lg:flex items-center gap-1 text-[14.5px] font-bold text-slate-400 h-full">
                <Link to={isCompanyUser ? "/company" : "/dashboard"} className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === (isCompanyUser ? '/company' : '/dashboard') ? 'text-white' : ''}`}>
                  {location.pathname === (isCompanyUser ? '/company' : '/dashboard') && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  {isCompanyUser ? 'Dashboard' : 'Home'}
                </Link>
                {!isCompanyUser && (
                  <>
                    ...
                    <Link to="/practice" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/practice' ? 'text-white' : ''}`}>
                      {location.pathname === '/practice' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                      Practice
                    </Link>
                  </>
                )}
                ...
                <Link to="/quizzes" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/quizzes' ? 'text-white' : ''}`}>
                  {location.pathname === '/quizzes' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Quizzes
                </Link>
              </div>
            ) : (
              <div className="hidden lg:flex items-center gap-1 text-[14.5px] font-bold text-slate-400 h-full">
                <Link to="/" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/' ? 'text-white' : ''}`}>
                  {location.pathname === '/' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Home
                </Link>
                <Link to="/about" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/about' ? 'text-white' : ''}`}>
                  {location.pathname === '/about' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  About
                </Link>
                <Link to="/products" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/products' ? 'text-white' : ''}`}>
                  {location.pathname === '/products' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Products
                </Link>
                <Link to="/solutions" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/solutions' ? 'text-white' : ''}`}>
                  {location.pathname === '/solutions' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Solutions
                </Link>
                <Link to="/resources" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/resources' ? 'text-white' : ''}`}>
                  {location.pathname === '/resources' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Resources
                </Link>
                <Link to="/pricing" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/pricing' ? 'text-white' : ''}`}>
                  {location.pathname === '/pricing' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Pricing
                </Link>
                <Link to="/contact" className={`px-4 py-2 rounded-full flex items-center hover:text-white transition-all relative ${location.pathname === '/contact' ? 'text-white' : ''}`}>
                  {location.pathname === '/contact' && <motion.div layoutId="navbg" className="absolute inset-0 bg-white/10 rounded-full" style={{ zIndex: -1 }} />}
                  Contact
                </Link>
              </div>
            )}
                      ...
                      >
                        <div className="bg-[#1e2330] border border-slate-800 rounded-b-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] min-w-[240px] overflow-hidden">
                          ...
                          <div className="flex flex-col text-[14px]">
                            <Link to={isCompanyUser ? "/company/profile" : "/profile"} onClick={() => setShowUserMenu(false)} className="py-2.5 text-slate-100 hover:text-brand-primary transition border-b border-slate-800/50 flex items-center justify-between group/item">
                            ...
                            >
                              Logout
                              <LogOut size={14} className="opacity-0 group-hover/item:opacity-100 transition-all -translate-x-2 group-hover/item:translate-x-0" />
                            </button>
                          </div>
                        </div>
                        </div>
      ...
      {!isAppPage && (
        <footer className="pt-16 pb-8 mt-20 bg-slate-950 border-t border-slate-900">
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12 mb-16">
              <div className="lg:col-span-5 pr-8">
                ...
                <div className="flex gap-4 mt-8">
                  <a href="#" className="w-10 h-10 bg-white rounded-full flex items-center justify-center hover:-translate-y-1 hover:shadow-lg hover:shadow-white/20 transition-all">
              ...
              <div className="lg:col-span-2 lg:col-start-7">
                ...
                <ul className="space-y-4 text-sm text-slate-400">
                  <li><Link to="/products" className="hover:text-brand-light transition">Products</Link></li>
                  <li><Link to="/solutions" className="hover:text-brand-light transition">Solutions</Link></li>
                  <li><Link to="/resources" className="hover:text-brand-light transition">Resources</Link></li>
                  <li><Link to="/pricing" className="hover:text-brand-light transition">Pricing</Link></li>
                </ul>
              </div>
              ...
              <div className="lg:col-span-2">
                ...
                <ul className="space-y-4 text-sm text-slate-400">
                  ...
                  <li><Link to="/contact" className="hover:text-brand-light transition">Rate Us</Link></li>
                </ul>
              </div>
            ...
            <div className="border-t border-slate-900/50 pt-8 flex items-center justify-center text-sm text-slate-500">
