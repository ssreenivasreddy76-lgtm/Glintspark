...
export function AdminChallenges() {
  ...
  const challengeQuestions = challenges.filter(c => c.isPractice === false);
  ...
  const processRows = async (rows: any[], defaultLanguages: string[]) => {
    ...
    for (const row of rows) {
      ...
      const rawHidden = getValue(['hidden test cases', 'hidden tests', 'test cases']);
      let parsedTestCases: {input: string; output: string}[] = [];
      if (rawHidden) {
        try {
          parsedTestCases = typeof rawHidden === 'string' ? JSON.parse(rawHidden) : rawHidden;
        } catch (e) {
      ...
      const newChallenge = {
        ...
        hiddenTestCases: typeof rawHidden === 'string' ? rawHidden : JSON.stringify(rawHidden || []),
    ...
    if (imported === 0 && rows.length > 0) {
      ...
      alert(`Successfully imported ${imported} challenges! (from ${rows.length} rows)`);
    }
  };
  ...
  const languageCounts = challengeQuestions.reduce((acc, q) => {
    const langs = q.allowedLanguages && q.allowedLanguages.length > 0 
      ? q.allowedLanguages 
  ...
  return (
    <div className="flex-1 flex flex-col p-8 h-full overflow-hidden">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-black text-slate-900 ">Challenges Management</h2>
          <p className="text-sm text-slate-500  mb-3">View standalone coding challenges grouped by topic.</p>
            ...
            {challengeQuestions.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-slate-500 ">
                  No challenges found. Add some to get started.
                </td>
              </tr>
            ) : (
                ...
                <tr key={q.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {((q.allowedLanguages && q.allowedLanguages.length > 0) ? q.allowedLanguages : Object.keys(languageLabels)).map((lang: string) => (
